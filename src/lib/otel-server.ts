...
// Configuration
const serviceName = "form-example-server"; // Next.js server-side
...
export function initOtel(): { tracer: Tracer; logger: Logger; meter: Meter } {
  try {
    ...
    logger.emit({
      ...
      severityText: "INFO",
      body: "OpenTelemetry Server SDK started",
    });
    ...
    fallbackLogger.emit({
      ...
      severityText: "ERROR",
      body: "Error starting OpenTelemetry Server SDK",
...
export function shutdownOtel(): void {
  try {
    ...
    fallbackLogger.emit({
      severityNumber: SeverityNumber.ERROR,
      severityText: "ERROR",
      body: "Error shutting down OpenTelemetry Server SDK",
      attributes: { error: (error as Error).message },
    });
    throw error;
  }
